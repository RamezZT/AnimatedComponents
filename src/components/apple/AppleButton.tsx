...
const AppleButton = () => {
  ...
  const EntryAnimation = async () => {
    ...
    await animate(
      "#blue-circle",
      { r: 22.5, opacity: 1, cy: 40.5 },
      { delay: 0.1, duration: 0.25, ease: "easeOut" }
    );
    const animation3 = animate(
      "#blue-inner-circle",
      { r: "16.5278", opacity: 1, cy: "40.4722" },
  ...
  const ExitAnimation = async () => {
    ...
    const animation9 = animate(
      "#gray-circle",
      { r: 0, opacity: 1 },
      { delay: 0.5, duration: 0.25, ease: "easeOut" }
    );
    setGTransform("translateX(0px)");
    const animation10 = animate(
      "#expanded-gray-circle",
      { opacity: 0 },
      { delay: 0.5 }
    );
  ...
  return (
    ...
    >
      ...
      >
        ...
        <g
          style={{
            ...
            transition: "transform 0.5s cubic-bezier(0.5, 1.25, 0.3, 1)",
          }}
          clip-path="url(#clip0_49_269)"
        >
          <circle
            id="blue-inner-circle"
            cx="121.528"
            cy="40.4722"
            r="0"
            fill="#0171E2"
      ...
      <button
        ...
        className="bg-white text-black mt-12 px-4 py-2 rounded-md"
      >
        Entry
      </button>
      <button
        onClick={ExitAnimation}
        className="bg-white text-black mt-4 px-4 py-2 rounded-md"
      >
        Exit
      </button>
    </div>
